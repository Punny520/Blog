<% if (theme.site_footer) { %>
  ...
  <footer class="footer clearfix"  itemscope itemtype="http://schema.org/WPFooter">
    ...
    <!-- 计算运行天数代码-->
    <span id="timeDate">载入中...</span>

    <script>
        // 设置网站创建时间
        const creationDate = new Date("2024-10-01T00:00:00");
        ...
        function updateDays() {
            ...
            // 更新页面显示
            document.getElementById("timeDate").innerHTML = `建站至今已 ${days} 天 `;
        }

        // 确保 DOM 完全加载后启动计时器
        document.addEventListener("DOMContentLoaded", function() {
            updateDays(); // 初始调用以立即显示天数
            setInterval(updateDays, 86400000); // 每天更新一次
        });
    </script>
    <!-- 计算运行天数代码-->
